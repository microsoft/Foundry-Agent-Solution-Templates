...
class TemplateConfigTests(unittest.TestCase):
    ...
    def test_explicit_empty_environment_does_not_inherit_process_values(self):
        with tempfile.TemporaryDirectory() as value, patch.dict(os.environ, {"CUSTOM_ENDPOINT": "https://unrelated.example"}):
            folder = Path(value)
            self.write(folder, "a.yaml", "name: a\nserver_url: ${CUSTOM_ENDPOINT}\n")
            with self.assertRaisesRegex(ValueError, "CUSTOM_ENDPOINT"):
                load_fragments(folder, {})
    ...
    def test_ownership_uses_current_list_shape(self):
        with tempfile.TemporaryDirectory() as value:
            path = Path(value) / "ownership.json"
            write_ownership(path, "https://search.example", "kb", ["ks"], "demo-index", [])
            state = json.loads(path.read_text(encoding="utf-8"))
            self.assertEqual(state["knowledgeBases"], ["kb"])
            self.assertEqual(state["knowledgeSources"], ["ks"])
            self.assertEqual(state["demoIndexes"], ["demo-index"])
