...
class InfrastructureContractTests(unittest.TestCase):
    ...
    def test_byo_postdeploy_error_is_actionable(self):
        hook = (ROOT / "scripts/postdeploy.ps1").read_text(encoding="utf-8")
        self.assertIn("az role assignment create --assignee-object-id $principalId", hook)
        self.assertIn("Then rerun: azd deploy --no-prompt", hook)
