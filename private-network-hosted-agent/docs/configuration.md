...
through the active azd environment. The default Terraform implementation is in
`infra-terraform`; the Bicep companion maps them through
...
| `AZURE_RESOURCE_GROUP` | Output | `rg-<environment-name>` | Dedicated group created by the selected implementation. It cannot be supplied or overridden. Use `scripts/cleanup.ps1` for ordered teardown. |
...
| `AZURE_AI_MODEL_DEPLOYMENT_NAME` | No | `gpt-5.1` | Model resource name. Both implementations and preflight allow only the documented model/version pair. |
| `AZURE_AI_MODEL_VERSION` | No | `2025-11-13` | Pinned model version. Changing it requires coordinated Terraform, Bicep, preflight, azd, test, and validation updates. |
...
Both `infra-terraform` and `infra-bicep` intentionally omit explicit zones on
Azure Firewall and its Standard public IP. In a region that supports
Availability Zones, new Firewalls without explicit zones are zone-redundant by
default; in other regions they remain regional. The `VpnGw2AZ` gateway uses a
zone-redundant Standard public IP across zones 1, 2, and 3, as required by the
VPN Gateway resource provider. These are Microsoft-managed platform behaviors,
not a template availability guarantee. See
...
| `S2S_SHARED_KEY` | S2S | Secret process environment variable only. Never use `azd env set`, source files, logs, or fixtures. The orchestrator includes only its SHA-256 digest in its own fingerprint. Terraform still records the connection input in its ignored local state and generated working files, so protect those files as secrets and remove the process variable after provisioning. The Bicep companion does not persist the clear-text key in azd state. |
...
The default Terraform ACR workflow uses
`scenarios/existing-private-acr/azure.yaml`. Its Bicep equivalent uses
`scenarios/bicep-existing-private-acr/azure.yaml`. Both require
