...
function Remove-ApimPolicyIfPresent([hashtable] $headers, [string] $policyUri) {
    try {
        Invoke-RestMethod -Headers $headers -Uri $policyUri | Out-Null
    }
    catch {
    ...
    Invoke-RestMethod -Method Delete -Headers $headers -Uri $policyUri | Out-Null
}

function Remove-ApimSubscriptionsForScope(
    [hashtable] $headers,
    [string] $apimId,
    [string] $scopeId
) {
    $subscriptionsUri = "https://management.azure.com${apimId}/subscriptions?api-version=2024-05-01"
    $subscriptions = @((Invoke-RestMethod -Headers $headers -Uri $subscriptionsUri).value)
    foreach ($subscription in @($subscriptions | Where-Object {
        Test-ResourceIdEqual $_.properties.scope $scopeId $false
    })) {
        Invoke-RestMethod `
            -Method Delete `
            -Headers $headers `
            -Uri "https://management.azure.com$($subscription.id)?api-version=2024-05-01" | Out-Null
    }
}
...
Remove-ApimSubscriptionsForScope $headers $apimId $productId
Remove-ApimPolicyIfPresent $headers "https://management.azure.com${apimId}/apis/${accountName}/policies/policy?api-version=2024-05-01"
Remove-ApimPolicyIfPresent $headers "https://management.azure.com${productId}/policies/policy?api-version=2024-05-01"

Write-Host 'Foundry AI Gateway links are reconciled and legacy subscriptions and model policies are removed.'