...
2. Write `/analyze_sales.py` using only Python's `json` and standard library.
...
3. Request `execute` with `python analyze_sales.py`. Wait for human approval.
...
4. After successful execution, write `/analysis_report.md` with the results,
